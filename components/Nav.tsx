    ...
    <div>
      <nav
        className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 h-[74px] ${
          isScrolled
            ? 'bg-[var(--bg-color)] backdrop-blur-lg shadow-[var(--scroll-shadow)]'
            : 'bg-[var(--bg-color)] backdrop-blur-md'
        }`}
      >
        <div className="max-w-[1200px] 2xl:max-w-[1440px] mx-auto p-4 flex justify-between items-center h-full">
          ...
          <div className="hidden md:flex items-center space-x-8">
            {['Hero', 'Products', 'Shop', 'Pages', 'Sale', 'Contact'].map(
              (item) => (
                <div
                  key={item}
                  className="text-[var(--text-color)] font-bold text-[1rem] cursor-pointer hover:text-[var(--hover-color)] transition-colors duration-200 ease-in font-primary"
                >
                  {item}
                </div>
              )
          ...
          <div className="hidden md:flex items-center space-x-4 cursor-pointer">
            ...
            <div className="bg-[var(--hover-color)] rounded-full p-2 cursor-pointer">
              <svg
                width="22px"
                height="22px"
                viewBox="0 0 24 24"
            ...
            <div className="rounded-full border-2 border-[var(--hover-color)] cursor-pointer">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="var(--text-color)"
                width="33px"
      ...
      {/* Mobile Menu */}
      {isMobileMenuOpen && (
        <div className="md:hidden flex flex-col items-center bg-[var(--bg-color)] backdrop-blur-md p-4 space-y-2 fixed top-[74px] left-0 w-full z-50">
          {['Hero', 'Products', 'Shop', 'Pages', 'Sale', 'Contact'].map(
            (item) => (
              <div
                key={item}
